...
export default class GameController {
    ...
    start(gameView: any, ui: UiElements, scoreBoard: ScoreBoard) {
        ...
        music.play().then(() => null)

        function move() {
            if (!isGame) {
                let crash = ui.crash()
                document.body.append(crash)
                crash.play().then(() => null)
                music.pause()
            ...
            let pipes = Array.from(document.querySelectorAll('.pipes') as unknown as HTMLCollectionOf<HTMLElement>);
            pipes.forEach((e) => {
